...
// GDI+ private font collection for custom TTF
static PrivateFontCollection* g_fontCollection = nullptr;
static FontFamily*             g_customFamily   = nullptr;
static wchar_t                 g_loadedFontPath[260] = {};

static void LoadCustomFont(const wchar_t* path) {
    if (wcscmp(path, g_loadedFontPath) == 0) return; // already loaded
    delete g_customFamily;    g_customFamily   = nullptr;
    delete g_fontCollection;  g_fontCollection = nullptr;
    if (path[0] == 0) { g_loadedFontPath[0]=0; return; }
    g_fontCollection = new PrivateFontCollection();
    if (g_fontCollection->AddFontFile(path) == Ok) {
        INT count = g_fontCollection->GetFamilyCount();
        if (count > 0) {
            FontFamily* fams = new FontFamily[count];
            INT found = 0;
            g_fontCollection->GetFamilies(count, fams, &found);
            if (found > 0) g_customFamily = new FontFamily(fams[0]);
            delete[] fams;
        }
    }
    wcsncpy_s(g_loadedFontPath, path, 259);
}
...
static BOOL WINAPI HookedWglSwapBuffers(HDC hDC)
{
    ...
    if (!g_glInit) {
        ...
        ReadConfig();
        if(g_oc.fontPath[0]){
            io2.Fonts->AddFontFromFileTTF(
                // convert wchar to char
                ([&](){static char buf[260];
                 WideCharToMultiByte(CP_UTF8,0,g_oc.fontPath,-1,buf,260,nullptr,nullptr);
                 return buf;})(),
                g_oc.fontSize>0?g_oc.fontSize:13.f);
        } else {
            ImFontConfig fc; fc.SizePixels=g_oc.fontSize>0?g_oc.fontSize:13.f;
            io2.Fonts->AddFontDefault(&fc);
        }
        ApplyRTSSStyle();
